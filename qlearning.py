...
class QLearning:
    ...
    def _save_table(self):
        io_handler.IOHandler.write(self._file_name + table_postfix,
    ...
    def __change_elements_in_state_history(self, state_key):
        ...
        self._last_20_before_last_20[self._20_list_idx] = \
            self._last_20[self._20_list_idx]
...
class BackwardQLearning(QLearning):
    ...
    def __resize_log(self):
        self.__s_a_log.append(get_np_array((BackwardQLearning.m, BackwardQLearning.n), int))
    ...
    def __update(self, state_key):
        ...
        if self._previous_state_key is not self._previous_action_key:
            reward = self.get_reward(
                              self._services.goal_tracking.reached_all_goals())
            self.__add_to_log(self._previous_state_key,
                              self._previous_action_key, reward)

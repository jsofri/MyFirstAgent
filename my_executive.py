...
def handle_known_problem(file_name):
    ...
    """
    ql_executive = qlearning.QLearningExecutive(file_name)
    mcts_executive = mcts.MctsExecutive(file_name)

    mcts_plan = mcts_executive.get_plan()
    ql_plan = ql_executive.get_plan()
    if mcts_plan is None and ql_plan is None:
        handle_unknown_problem()
        return
    elif mcts_plan is None:
        executive = ql_executive
    elif ql_plan is None or len(ql_plan) / 3 > len(mcts_plan):
        executive = mcts_executive
    else:
        executive = ql_executive
